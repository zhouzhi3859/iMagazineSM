...
@Component({
  selector: 'staitcsum-component',
  templateUrl: './static-sum.component.html',
  styleUrls: ['./static-sum.component.css', ],
}, )
export class StaticSumComponent implements OnInit {
  constructor(
  ...
  public showStatic() {
    this.articleService.getStatic(this.day, ).subscribe((res, ) => {
      // 组合数据
      const collectData = [];
      const commontData = [];
      const readData = [];
